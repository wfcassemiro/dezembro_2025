        ...
        if (empty($subject) || empty($message_body)) {
            ...
            try {
                ...
                if ($recipient_type === 'selected' && !empty($selected_users)) {
                    ...
                    // NOVO: Buscar leads da agenda que podem receber email
                    $stmt = $pdo->query("SELECT id, email, nome as name FROM leads WHERE COALESCE(pode_receber_email, 1) = 1");
                    $recipients = $stmt->fetchAll();
                }
...
// Contar leads da agenda
try {
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM leads WHERE COALESCE(pode_receber_email, 1) = 1");
    $result = $stmt->fetch();
    $total_leads = $result ? $result['total'] : 0;
} catch (PDOException $e) {
    $db_errors[] = "Erro ao contar leads: " . $e->getMessage();
    $total_leads = 0;
}
...
<div class="main-content">
    ...
    <div class="video-card glass-card">
        ...
        <div class="stats-row">
            ...
            <div class="stat-item stat-pink">
                ...
                <div class="stat-label">Leads da Agenda</div>
            </div>
    ...
    <div class="video-card glass-card">
        ...
        <div class="quick-actions-grid">
            ...
            <div class="quick-action-card" onclick="useTemplate('leads_sorteio')">
                <div class="quick-action-icon" style="color: #ec4899;"><i class="fas fa-gift"></i></div>
                <h4>Sorteio Leads</h4>
            </div>
...
Equipe Translators101`
    ...
    leads_sorteio: {
        subject: '🎁 Resultado do Sorteio - Translators101',
        message: `Olá [NOME],

Temos uma novidade para você sobre o sorteio mensal da Translators101!

Você foi sorteado(a) e ganhou acesso gratuito à nossa plataforma por 1 mês!
...
.stat-item.stat-pink .stat-number {
    color: #ec4899;
}
