...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    try {
        if ($_POST['action'] === 'mark_job_as_paid') {
            $job_id = $_POST['job_id'];

            if (!empty($job_id)) {
                // Atualiza o status do trabalho para 'paid'
                $stmt = $pdo->prepare("UPDATE dash_jobs SET status = 'paid' WHERE id = ? AND user_id = ? AND status = 'completed'");
                $stmt->execute([$job_id, $user_id]);
                
                $_SESSION['temp_message'] = "Custo/Trabalho marcado como pago!";
                header("Location: report_accounts_payable.php");
                exit;
            }
        }
    } catch (PDOException $e) {
        $error = "Erro ao atualizar o trabalho: " . $e->getMessage();
    }
}
...
$job_status_list = [
    ...
    'completed' => 'Pendente de Pagamento', // Concluído, mas não pago
    'paid' => 'Pago'
];
...
try {
    ...
    foreach ($stmt_rates->fetchAll(PDO::FETCH_ASSOC) as $row) {
        ...
        if((float)$row['setting_value'] > 0) {
            ...
            $rates_string_parts[] = "1 $code = " . (float)$row['setting_value'] . " $base_currency";
        }
    }
    ...
    $sql = "
        ...
        WHERE j.user_id = ?
          AND j.status = 'completed' -- Busca apenas jobs 'Concluídos' (pendentes de pagamento)
    ...
    foreach ($report_data as $job) {
        $amount_brl = $convert_to_brl($job['total_cost'], $job['currency']);
        
        if ($amount_brl === null) {
            $missing_rates_warning = true;
            continue; // Pula este job se a taxa de câmbio estiver faltando
        }

        $total_payable_brl += $amount_brl;
...
$page_description = "Custos de fornecedores (jobs) concluídos e pendentes de pagamento.";
...
<div class="main-content">
    ...
    <div class="video-card profile-header-card" style="background: linear-gradient(135deg, var(--brand-purple), #4a148c); border: none; margin-bottom: 25px;">
        <div class="header-icon-container">
            <i class="fas fa-hand-holding-usd" style="font-size: 1.8rem; color: #fff;"></i>
        </div>
    ...
    <div class="video-card">
        ...
        <div class="kpi-grid" style="padding: 20px 30px;">
            ...
            <div class="kpi-card-media kpi-cost"> <h4><i class="fas fa-dollar-sign"></i> Total a Pagar</h4>
                <h3><?php echo formatCurrency($total_payable_brl, $base_currency); ?></h3>
            </div>
        </div>
    ...
    <div class="video-card">
        <div class="card-header-refined">
            <h2><i class="fas fa-list-ul"></i> Detalhamento de Custos Pendentes (Jobs)</h2>
            <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                
                <a href="generate_accounts_payable_pdf.php" 
                   id="btn_export_pdf_payable" 
                ...
                <button id="btn_export_csv_payable" class="vision-btn vision-btn-secondary" <?php echo $hasData ? '' : 'disabled'; ?>>
        ...
        <div class="vision-table-container">
            <table class="vision-table" id="report_table">
                <thead>
                    <tr>
                        <th><i class="fas fa-user-tie"></i> Fornecedor</th>
                        <th><i class="fas fa-project-diagram"></i> Projeto</th>
                        <th><i class="fas fa-money-bill-wave"></i> Custo</th>
                ...
                <tbody>
                    <?php if (empty($report_data)): ?>
                        <tr>
                            <td colspan="6" style="text-align: center;">Nenhum custo pendente de pagamento encontrado.</td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($report_data as $job): ?>
                            <?php
                                $deadline_status = 'future';
                                if ($job['deadline'] && $job['deadline'] < $today_str) {
                                    $deadline_status = 'overdue';
                                }
                            ?>
                            <tr>
                                <td class="project-name-refined"><?php echo htmlspecialchars($job['freelancer_name']); ?></td>
                                <td><?php echo htmlspecialchars($job['project_title']); ?></td>
                                <td class="value-cell-refined" style="color: var(--accent-orange);">
                                ...
                                <td class="<?php echo $deadline_status === 'overdue' ? 'text-danger' : ''; ?>" style="font-weight: <?php echo $deadline_status === 'overdue' ? '600' : 'normal'; ?>;">
                                    <?php echo $job['deadline'] ? date('d/m/Y', strtotime($job['deadline'])) : 'N/A'; ?>
                                ...
                                <td>
                                    <span class="status-badge-refined status-pending">
                                        <?php echo htmlspecialchars($job_status_list[$job['status']] ?? $job['status']); ?>
                                    </span>
                                </td>
                                <td>
                                    <button class="vision-btn vision-btn-primary btn-mark-paid" 
                                            data-id="<?php echo $job['id']; ?>" 
                                            data-title="<?php echo htmlspecialchars($job['freelancer_name'] . ' - ' . $job['project_title']); ?>"
    ...
    <div class="vision-modal-content" style="max-width: 500px;">
        ...
        <form method="POST" action="report_accounts_payable.php" class="vision-form" style="padding: 20px 30px 30px;">
            <input type="hidden" name="action" value="mark_job_as_paid">
            <input type="hidden" name="job_id" id="modal_job_id">
            
            <p style="color: var(--text-secondary); margin-bottom: 20px;">Você confirma o pagamento do custo/job <strong id="modal_job_title" style="color: var(--text-primary);"></strong>?</p>
...
<script>
    ...
    function exportPayableTableToCSV(filename) {
        let csv = [];
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const btnCsv = document.getElementById('btn_export_csv_payable');
        if (btnCsv) {
            btnCsv.addEventListener('click', function() {
                if (this.disabled) {
                    return;
                }
                exportPayableTableToCSV('contas_a_pagar.csv');
            });
        }

        const btnPdf = document.getElementById('btn_export_pdf_payable');
        if (btnPdf) {
