/**
 * Time Tracker - JavaScript V3 (Final - Cache Buster)
 * Lógica visual corrigida
 */

console.log('%c VERSÃO V3 (NOVO ARQUIVO) CARREGADA ', 'background: #222; color: #00ff00; font-size: 20px');

// Detecção robusta do caminho da API
const currentPath = window.location.pathname.substring(0, window.location.pathname.lastIndexOf('/'));
// Garante que a API seja buscada na mesma pasta onde o PHP está rodando
...
function startTimer() {
    ...
    .then(data => {
        showLoading(false);
        if (data.success) {
            console.log('[TT] Start Sucesso:', data.entry);
            if (data.entry) {
...
function resumeTimer() {
    ...
    fetch(API_URL, { method: 'POST', body: fd }).then(r=>r.json()).then(d=>{
        if(d.success) { checkRunningTimer(); state.isPaused = false; updateTimerUI(); }
    });
}
