<?php
// Debug: Verificar se o arquivo database.php existe
$database_path = __DIR__ . '/../config/database.php';
if (!file_exists($database_path)) {
    die("❌ Erro: Arquivo database.php não encontrado em: $database_path");
}

// Incluir configuração do banco e iniciar sessão
require_once $database_path;

// Debug: Verificar se as funções existem
if (!function_exists('hasVideotecaAccess')) {
    die("❌ Erro: Função hasVideotecaAccess não encontrada. Verifique o arquivo database.php");
}

if (!function_exists('isLoggedIn')) {
    die("❌ Erro: Função isLoggedIn não encontrada. Verifique o arquivo database.php");
}
...
if (isset($_GET['debug'])) {
    ...
    echo "hasVideotecaAccess(): " . (hasVideotecaAccess() ? 'true' : 'false') . "\n";
    echo "==================\n";
    echo "</pre>";
    if (!hasVideotecaAccess()) {
        echo "<p style='color: red;'>❌ Acesso negado - redirecionando...</p>";
        echo "<p><a href='?'>Tentar novamente</a></p>";
        exit;
    }
}
...
if (!hasVideotecaAccess()) {
    // Em vez de redirecionar imediatamente, vamos mostrar uma mensagem de debug
    if (isset($_GET['force'])) {
        // Forçar acesso para debug (remover em produção)
        echo "<div style='background: #ffe6e6; padding: 10px; margin: 10px; border: 1px solid red;'>";
        echo "⚠️ AVISO: Acesso forçado para debug. Usuário não está logado corretamente.";
        echo "</div>";
    } else {
...
if (isLoggedIn()) {
    ...
    if (!empty($user_id)) {
        try {
            ...
            // Ignorar erro para nao quebrar a pagina
        }
    }
}
...
.featured-tag {
    ...
    text-transform: capitalize !important; /* Primeira letra maiuscula */
}
    ...
    <div class="videoteca-filtros">
        ...
        <form method="GET" action="" class="filtros-form">
            <div class="filtros-grid-improved">
                ...
                <!-- Campo de Ordenacao - MODIFICADO: opcao "Temporada (decrescente)" como primeira e padrao selecionado -->
            ...
            ?>
                ...
                <div class="video-card" onclick="location.href='/palestra.php?id=<?php echo $lecture['id']; ?>'">
                    ...
                    <div class="video-info">
                        ...
                        <!-- Checkbox para adicionar/remover da watchlist (apenas para usuarios logados) -->
                        <?php if (isLoggedIn()): ?>
...
function toggleWatchlist(checkbox, lectureId) {
    ...
    // Faz a requisicao AJAX para atualizar no servidor
