...
// Usando o Product ID como parâmetro
$api_url = HOTMART_API_BASE_URL . "/hotclub/rest/v2/page/list?product_id=" . urlencode(HOTMART_PRODUCT_ID);
...
if ($api_call_result['error']) {
    ...
    $output .= "<p>Todas as tentativas de API falharam. A causa é o Subdomínio incorreto ou uma restrição de rede no servidor que impede a conexão com o host de dados da Hotmart.</p>";
...
<div class="container">
    ...
    <?php if ($total_pages_count === 0): ?>
        <div class="alert-warning alert-message">
            A API da Hotmart retornou dados, mas a lista de páginas/aulas está vazia (<?php echo count($hotmart_lessons); ?> módulos no nível superior).
        </div>
        ...
        <?php if (!empty($hotmart_lessons_response)): ?>
            ...
            <p>Se o JSON acima **não for vazio**, o problema é na extração, caso contrário, é a URL ou o ID do Produto.</p>
        <?php else: ?>
             <p>A API retornou uma resposta vazia (<code>null</code> ou <code>false</code>) ou não retornou módulos no nível superior. Verifique o **ID do Produto** (<?php echo HOTMART_PRODUCT_ID; ?>) e o **Host de API**.</p>
        <?php endif; ?>
